...
def extract_locations_from_docs(docs: List[Document]) -> List[dict]:
    ...
    for doc in docs:
        ...
        locations.append({
            "name": meta.get("캠핑장이름", "이름 정보 없음"),
            "address": meta.get("캠핑장주소", "주소 정보 없음"),
            "latitude": meta.get("캠핑장 위도"),
            "longitude": meta.get("캠핑장 경도"),
        })
    return locations
...
async def search_paid_camping(state: GraphState) -> dict:
    ...
    try:
        ...
        print(f"유료캠핑장 검색 결과: {len(docs)}개 문서")
...
async def search_glamping_caravan(state: GraphState) -> dict:
    ...
    try:
        ...
        context = docs.copy()
        locations = extract_locations_from_docs(docs)
        print(f"글램핑/카라반 RAG 검색 결과: {len(docs)}개 문서")
...
async def search_ojee_camping(state: GraphState) -> dict:
    ...
    try:
        ...
        locations = extract_locations_from_docs(docs)
        print(f"오지/노지캠핑 검색 결과: {len(docs)}개 문서")
...
async def general_web_search_async(query: str, camping_type: str) -> List[str]:
    ...
    return random.sample(mock_results.get(camping_type, []), k=2)
# ===== 노드 8: 장소추천 답변 생성 =====
...
답변:"""
    ...
    try:
        response = llm_pipeline_main(prompt, max_new_tokens=512)[0]['generated_text']
        final_answer = response.replace(prompt, "").strip()
        return {"final_answer": final_answer, "locations": locations}
    except Exception as e:
        return {"final_answer": "장소 추천 답변 생성 중 오류가 발생했습니다.", "error_message": str(e), "locations": locations}
...
workflow.add_node("classify_camping", classify_camping_type)         # 노드4: 캠핑 유형 분류 (3가지)
workflow.add_node("search_paid", search_paid_camping)               # 노드5: 유료캠핑장 검색
workflow.add_node("search_glamping", search_glamping_caravan)        # 노드6: 글램핑/카라반 검색
workflow.add_node("search_ojee", search_ojee_camping)               # 노드7: 오지/노지 검색
workflow.add_node("generate_location", generate_location_answer)     # 노드8: 장소 답변
